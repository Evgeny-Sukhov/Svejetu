...
switch ($request_mode) {
    case 'listing':
        ...
        $listings = $rlSearch->search($data, false, false, 100);
    ...
    case 'placesAutocomplete':
        ...
        $data = Util::geocoding($query, false, null, $provider);
    ...
    case 'geocoder':
        $params   = json_decode($_REQUEST['params'], true) ?: $_REQUEST['params'];
        $provider = $config['geocoding_provider'];

        if (in_array($params['provider'], ['nominatim', 'googlePlaces'])) {
