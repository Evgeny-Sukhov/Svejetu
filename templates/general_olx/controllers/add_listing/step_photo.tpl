...
<script id="uploaded_picture" type="text/x-jsrender">
    <div class="picture-cont {$column_model} [%:Type%][%if Original == 'youtube'%] youtube[%/if%][%if error%] template-upload file-error[%/if%]" data-id="[%:ID%]" data-source="[%:Original%]">
        <span>
            <span class="item">
                [%if error %]
...
var flMediaManager = function(){
    ...
    var picture_file_types = 'gif|jpe?g|png';
            ...
            $media_manager.on('click', '.icon.crop', function(){
                ...
                var $interface = $('<div>')
                    ...
                    .append(
                        $('<img>')
                            .addClass('crop-image')
                            .attr('src', $container.data('source').replace(/https?:/, ''))
                            .css('maxWidth', '100%')
                            .on('load', function(){
