...
{if $current_box_categories}
    ...
    {/if}
    <div class="categories{if $box_listing_type.Ablock_show_subcats} once sub-categories-exist{/if}" id="categories_{$box_listing_type_key|replace:'_':''}_{$pages_number}">
        <ul class="clearfix">
            <li>
                ...
                {foreach from=$current_box_categories item='cat' name='fCats'}
                    ...
                    {if $cat.Ablock_position}
                        ...
                        {assign var='box_listing_type' value=$cat}
                    {else}
