		...
		<div class="area_profile step_area">
			<form name="account_reg_form" method="post" action="{$rlBase}{if $config.mod_rewrite}{$pageInfo.Path}.html{else}?page={$pageInfo.Path}{/if}" enctype="multipart/form-data">
				...
				<div class="submit-cell">
					...
					<div class="field single-field">
						<div><input size="45" class="wauto" type="text" name="profile[mail]" {if $smarty.session.registration.account_id}readonly="readonly"{/if} {if $smarty.post.profile.mail}value="{$smarty.post.profile.mail|htmlspecialchars}"{/if} /></div>
						<div class="options">
					...
					<div class="submit-cell">
						...
						<div class="field single-field">
							{if $config.account_wildcard}
								{$scheme}://<input type="text" style="width: 90px;" maxlength="30" name="profile[location]" {if $smarty.session.registration.account_id}readonly="readonly"{/if} {if $smarty.post.profile.location}value="{$smarty.post.profile.location}"{/if} />.{$domain}
							{else}
