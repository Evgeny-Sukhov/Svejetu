...
abstract class AbstractSteps
{
    ...
    public function init(&$page_info = null, &$account_info = null, &$errors = null)
    {
        ...
        $GLOBALS['rlSmarty']->register_function('processStep', array($this, 'processTplStep'));
        $GLOBALS['rlSmarty']->register_function('buildPrevStepURL', array($this, 'buildPrevStepURL'));
        $GLOBALS['rlSmarty']->register_function('buildFormAction', array($this, 'buildFormAction'));
    }
    ...
    public function getStep()
    {
        if ($GLOBALS['config']['mod_rewrite']) {
            $step = array_pop(explode('/', $_GET['rlVareables']));
    ...
    public function processTplStep()
    {
         ...
         * @since 4.6.0
         */
        $GLOBALS['rlHook']->load('stepsProcessTpl', $this, $tpl, $step);
