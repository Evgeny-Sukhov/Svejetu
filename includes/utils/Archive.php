    ...
    public static function unpack($archive, $destination, $removeArchive = true)
    {
        ...
        if (class_exists('ZipArchive')) {
            ...
            try {
                ...
                $zipFile->openFile($archive);
                $zipFile->extractTo($destination);
                $result = true;
            } catch(\PhpZip\Exception\ZipException $e){
                $rlDebug->logger("PhpZip | Archive cannot be unpacked, error code: " . $e->getMessage());
                $result = false;
            } finally{
                $zipFile->close();
            }
            ...
            try {
                ...
                foreach ($files as $file) {
                    if (is_file($file)) {
                        ...
                        $zipFile->addDirRecursive($file, str_replace($folderName, '', $file));
                    }
                }

                $zipFile->saveAsFile($archive);
            } catch(\PhpZip\Exception\ZipException $e){
                $rlDebug->logger("PhpZip | Archive cannot be packed, error code: " . $e->getMessage());
            } finally{
                $zipFile->close();
            }
        }
