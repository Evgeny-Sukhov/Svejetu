...
class ListingMedia
{
    ...
    public static function addYouTube($listing_id, $link, &$account_info, &$plan_info, $position = 0)
    {
        ...
        if (0 === strpos($link, 'http')) {
            if (false !== strpos($link, 'youtu.be')) {
                $youtube_id = array_pop(explode('/', $link));
            } else {
        ...
        if ($youtube_id) {
            ...
            $check_url = "https://www.youtube.com/oembed?format=json&url=http://www.youtube.com/watch?v=" . $youtube_id;
    ...
    public static function cropPicture($listing_id, $media_id, $data = null, &$account_info)
    {
        ...
        $picture_data = pathinfo($item['Original']);
        $name_hash = time() . mt_rand();
        $cropped_picture = RL_UPLOAD . 'tmp_' . $name_hash . '.' . $picture_data['extension'];

        $original = RL_FILES . $item['Original'];

        $GLOBALS['rlHook']->load('listingMediaCropPicturePre', $listing_id, $item, $original, $data, $account_info);
        ...
        $GLOBALS['rlCrop']->saveImage($cropped_picture, $config['img_quality']);
        $GLOBALS['rlCrop']->flushImages();

        $upload_obj = new ListingPictureUpload($listing_id);
        ...
        if (is_readable($cropped_picture)) {
            ...
            foreach ($image_versions as $version => $options) {
                ...
                $new_file_name = $picture_data['dirname'] . '/' . $new_file_name . '.' . $picture_data['extension'];
                $new_file_path = RL_FILES . $new_file_name;
                $source_file   = $cropped_picture;

                // Keep original picture aspect ratio for large version
                if (!$config['img_crop_module'] && $config['img_crop_thumbnail'] && $version == 'large') {
            ...
            $GLOBALS['rlDebug']->logger("Unable to crop an original picture, destination file is unreadable: {$cropped_picture}");
        ...
        if (is_readable(RL_FILES . $results['Thumbnail'])) {
            ...
            foreach ($image_versions as &$version_options) {
                unlink(RL_FILES . $item[$version_options['db_field']]);
            }
            ...
            $GLOBALS['rlDebug']->logger("Unable to complete the crop task, unable to create picture variants");
     ...
     * @since 4.7.0 - moved from ListingPictureUpload
     * 
     * @param int $listing_id - id of the listing to update names of
     */
    public static function updateNames($listing_id)
    {
        global $rlDebug, $rlDb;
        ...
        $sql = "SELECT * FROM `{db_prefix}listing_photos` ";
        $sql .= "WHERE `Listing_ID` = {$listing_id}";
        $listing_photos = $rlDb->getAll($sql);

        $update = array();
        foreach ($listing_photos as $key => $photo) {
            $upd = array();
            ...
            if ($photo['Type'] == 'picture') {
                $img_options = $options['image_versions'];
            } else {
                $img_options = array();
            }
            ...
            foreach ($img_options as &$option) {
                ...
                $upd['fields'][$db_field] .= self::buildName(
                    $listing_id,
                    $option['prefix'],
                    $photo['Position'],
                    $fileInfo['extension'],
                ...
                $upd['fields'][$db_field] .= '.' . $fileInfo['extension'];
    ...
    public static function getOptions()
    {
        ...
        $options = array(
            'picture_file_types'          => '/(\.|\/)(gif|jpe?g|png)$/i',
            'video_file_types'            => '/(\.|\/)(mp4|webm|ogg)$/i',
            // Defines which files (based on their names) are accepted for upload
            'accept_file_types'           => '/(\.|\/)(gif|jpe?g|png|mp4|webm|ogg)$/i',
            // Extensions string to appear in error messages
            'accept_file_ext'             => 'gif/jpeg/png/mp4/webm/ogg',
     ...
     * @since 4.7.1
     *
     * @param  array $picture - Picture database enty data
    ...
    public static function updatePicture($picture)
    {
        ...
        if (is_readable($source_file)) {
            ...
            foreach ($image_versions as $version => $options) {
                ...
                $GLOBALS['rlResize']->resize(
                    $options['force_crop'] ? $new_file_path : $source_file,
                    $new_file_path,
                    'C',
                    array($options['max_width'], $options['max_height']),
                    $options['force_crop'],
