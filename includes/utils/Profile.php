...
class Profile
{
    ...
    public static function getPersonalAddress($account, $type = array(), $langCode = '')
    {
        ...
        $url     = $rlAccount->isAdmin() ? RL_URL_HOME : $GLOBALS['reefless']->getPageUrl('home', false, $langCode);
        ...
        if ($type['Own_location'] && $address && $config['mod_rewrite']) {
            ...
            $path = $pages['at_' . ($type['Key'] ?: $type['Type'])];

            if ($config['mod_rewrite']) {
                $url = "{$scheme}://{$url}{$path}/{$address}/";
            } else {
