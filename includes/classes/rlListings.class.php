...
class rlListings extends reefless
{
    ...
    {
        global $category, $rlCommon, $account_info, $location, $rlHook, $rlActions, $rlValid, $rlPlan;
    ...
    {
        global $config, $rlCommon, $rlHook, $location, $account_info, $rlValid, $rlActions, $rlPlan, $rlDb;
     ...
     * @return array - listings information
     **/
    public function getListingsByAccount($account_id = false, $order = false, $order_type = 'ASC', $start = 0, $limit = false)
    {
    ...
    public function getRecentlyAdded($start = 0, $limit = 0, $listing_type = '')
    {
        ...
        $sql .= "ORDER BY `T1`.`{$date_field}` DESC ";
        $sql .= "LIMIT {$start}, {$limit}";
    ...
    public function getMyListings($type = '', $order = 'ID', $order_type = 'asc', $start = 0, $limit = 0)
    {
        ...
        $sql .= "AND `T1`.`Status` NOT IN ('trash', 'approval') AND `T4`.`Status` = 'active' ";
    ...
    public function getListingDetails($id, &$listing, $listing_type = false)
    {
        ...
        $display_posted_date = (bool) $config['display_posted_date'] && !$tpl_settings['ld_posted_date_fixed'];
    ...
    public function ajaxTellFriend(
        ...
        $security_code = '',
        $listing_id) {
        global $_response, $page_info, $lang, $rlMail;
    ...
    public function deleteListing($id, $account_id = false)
    {
        $id = (int) $id;
    ...
    public function getFeatured($type = '', $limit = 10, $field = '', $value = '', $block_key = '')
    {
        ...
        $sql .= "WHERE `T1`.`Featured_date` AND `T1`.`Status` = 'active' ";
        ...
        $sql .= 'ORDER BY ' . ($category['ID'] ? '`Category_match` DESC, ' : '') . "`Last_show` ASC ";
        $sql .= "LIMIT {$start}, {$limit}";
     ...
     *
     **/
    public function purchasePackage($id, $plan_id = false, $account_id, $free = false)
    {
    ...
    public function uploadVideo($type = false, $source = false, $listing_id = false)
    {
        ...
        switch ($type) {
            ...
            case 'youtube':
                ...
                $check_url = "http://www.youtube.com/oembed?format=json&url=http://www.youtube.com/watch?v=" . $matches[1];
                $ut_video = $GLOBALS['rlJson']->decode($this->getPageContent($check_url), true);
    ...
    public function originalUrlRedirect($mode = 'listing', $data = array())
    {
        ...
        switch ($mode) {
            ...
            case 'listing':
                ...
                $parsed    = $config['multilingual_paths'] ? Util::parseURL($real_url) : parse_url($real_url);
                $real_uri  = ltrim($parsed['path'], '/');
                $real_base = $parsed['scheme'] . '://' . $parsed['host'] . '/';
     ...
     * @param bool $home_page - is home page
     ...
     **/
    public function getListingsByLatLng($type = false, $start = 1, $coordinates = array(), $form = false, $home_page = false, $group_search = false, $group_lat = false, $group_lng = false)
    {
        global $config, $data;

        if (!$type) {
            return array('listings' => null, 'count' => 0);
        }

        define('RL_SEARCH_ON_MAP', true);

        $form_key = $home_page ? 'search_map_home' : 'search_map';
