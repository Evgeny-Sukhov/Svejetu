...
class ManageListing extends AbstractSteps
{
    ...
    protected function simulateListingInPost($hook_name = 'addListingPostSimulation')
    {
        ...
        foreach ($this->formFields as &$field) {
            ...
            switch ($field['Type']) {
                case 'mixed':
                    $value = false;
                    $value = explode('|', $listing[$field['Key']]);
                ...
                case 'price':
                    $price = false;
                    $price = explode('|', $listing[$field['Key']]);
                ...
                case 'unit':
                    $unit = false;
                    $unit = explode('|', $listing[$field['Key']]);
    ...
    {
        global $rlSmarty, $page_info, $rlHook, $rlAccountTypes;
    ...
    protected function fetchPlans(&$account_info)
    {
        ...
        if ($this->planType == 'account') {
            if ($_SESSION['account'] && $account_info['Plan_ID']) {
                $this->plans[] = $rlMembershipPlan->getPlanByProfile($account_info);
            } else {
