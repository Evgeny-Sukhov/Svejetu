...
class rlAdmin extends reefless
{
    ...
    {
        global $lang, $rlHook, $l_timezone, $tpl_settings, $rlLang, $rlDb;
         ...
         * @var array
         */
        $systemSelects = array('timezone', 'map_provider', 'static_map_provider', 'geocoding_provider',
                               'search_map_location_zoom', 'map_default_zoom');
            ...
            switch ($value['Key']) {
                ...
                case 'address_on_map':
                    if ($GLOBALS['config']['address_on_map']) {
                        ...
                        if (!isset($GLOBALS['config']["amp_divider"])) {
                            ...
                            $GLOBALS['config']["amp_divider"] = $lang_val['Code'];
                ...
                case 'home_gallery_box':
                    if ($tpl_settings['home_page_gallery']) {
                        ...
                        $where .= "OR `T1`.`Plugin` = 'listings_box')";
    ...
    public function ajaxDeleteAccount($id = false, $reason = false, $direct = false)
    {
        ...
        if ($replace_id) {
            if ($replace_id == $id) {
                ...
                /* refresh listing count of a recepient */
                $sql = "UPDATE `{db_prefix}accounts` SET `Listings_count` = ( ";
                $sql .= "SELECT COUNT(*) FROM `{db_prefix}listings` AS `T1` ";
                $sql .= "WHERE `T1`.`Status` = 'active' AND ";
                $sql .= "`T1`.`Account_ID` = `{db_prefix}accounts`.`ID`";
                $sql .= ") WHERE `ID` = {$replace_id}";

                $this->query($sql);
                /* refresh listing count of a recepient end */
            }
        }
     ...
     *
     **/
    public function deleteTrashItem($id = false)
    {
        if (!$id) {
            return false;
        }
        ...
        if (empty($trash_item['Criterion'])
            || (empty($trash_item['Key']) && $trash_item['Zones'] != 'listings' && $trash_item['Zones'] != 'saved_search')) {
        ...
        foreach ($tables as $table) {
            ...
            switch ($table) {
                ...
                case 'tmp_categories':
                    $this->query("DELETE FROM `{db_prefix}tmp_categories` WHERE {$trash_item['Criterion']} LIMIT 1");
                    break;
                ...
                case 'data_formats':
                    ...
                    foreach ($child_keys as $cKey => $cVal) {
                        $this->query("DELETE FROM `{db_prefix}lang_keys` WHERE `Key` = 'data_formats+name+" . $child_keys[$cKey]['Key'] . "'");
                    }
        ...
        if (!empty($trash_item['Lang_keys'])) {
            ...
            if (!empty($lang_keys)) {
                ...
                $this->query("DELETE FROM `{db_prefix}lang_keys` WHERE {$where}");
            }
        }
     ...
     * @package xAjax
     *
     * @param string $ids     - listings ids, ex: 250,4023,501,...
