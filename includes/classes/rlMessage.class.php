...
class rlMessage extends reefless
{
    ...
    {
        global $_response, $config, $account_info, $rlSmarty, $pages, $rlMail, $reefless, $rlDb, $rlAccount, $rlHook;
        ...
        if (defined('IS_LOGIN') || defined('REALM')) {
            ...
            if ($config['messages_notification_in_chat']) {
                ...
                $mail_tpl = $rlMail->getEmailTemplate('contact_owner_user');
    ...
    public function getContacts()
    {
        ...
        $account_id = intval($account_id ?: $account_info['ID']);
        ...
        $sql .= "GROUP BY `T1`.`ID` ORDER BY IF(`T1`.`Status` = 'new', 0, 1) ASC, `ID` DESC ";
            ...
            // list of contacts for user in frontend
                ...
                if ($contacts[$value_by_index]) {
                    ...
                    if ($value['From'] == -1) {
                        ...
                        $messages[$key]['Name'] = $messages[$key]['Name'] ?: $this->getOne(
                            'Name',
    ...
    public function getMessages($user_id = false, $no_update = false, $visitor_mail = false, $admin = false)
    {
        ...
        foreach ($messages as $key => $value) {
            ...
            if ($value['Listing_ID']) {
                ...
                $messages[$key]['listing_url'] = $this->url('listing', $listing_info);
    ...
    public function ajaxRemoveMsg($ids = false, $contact_id = false, $admin = false)
    {
        ...
        if (!empty($ids[0])) {
            ...
            foreach ($ids as $id) {
                ...
                if ($res = $this->fetch($select, null, $where, 1, 'messages', 'row')) {
                    if ($res['From'] == -1 || $res['Remove']) {
                        //in visitor mode delete completely
                        $this->query("DELETE FROM `{db_prefix}messages` WHERE `ID` = {$id}");
                    } else {
                        if ($GLOBALS['rlAccount']->isAdmin()) {
            ...
            foreach ($ids as $contact_id) {
                ...
                foreach ($messages as $key => $value) {
                    if ($contact_visitor_email || $value['Remove']) {
                        $this->query("DELETE FROM `{db_prefix}messages` WHERE `ID` = '{$value['ID']}' LIMIT 1");
                        $deleted = true;
                    } else {
    ...
    {
        global $config, $lang, $rlMembershipPlan, $rlAccount, $rlListingTypes, $account_info, $pages, $attach_file;
        ...
        if ($listing_id) {
            ...
            $link = $this->url('listing', $info, $info['Lang']);
        ...
        if ($rlAccount->isLogin() || $config['messages_save_visitor_message']) {
            ...
            if ($config['messages_notification'] && $rlAccount->isLogin()) {
                ...
                $reply_link = $this->getPageUrl('my_messages', null, $info['Lang']);
