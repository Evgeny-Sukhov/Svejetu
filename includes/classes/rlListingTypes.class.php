...
class rlListingTypes extends reefless
{
     ...
     **/
    public function statisticsBlock()
    {
    ...
    public function ajaxDeletingType($key = false, $reason = false, $replace_key = false)
    {
        ...
        if ($replace_key) {
            $this->query("UPDATE `{db_prefix}categories` SET `Type` = '{$replace_key}' WHERE `Type` = '{$key}'");
        } else {
     ...
     *
     **/
    public function trashListingTypeData($key = false)
    {
        if (!$key) {
            return false;
        }
