...
class rlCommon extends reefless
{
    ...
    public function adaptValue(
        ...
        switch ($field['Type']) {
            ...
            case 'mixed':
                ...
                $number = $field['Opt1'] ? number_format($df[0], null, null, $thousands_sep) : $df[0];

                if (!empty($field['Condition'])) {
                    $out = $number . ' ' . $lang['data_formats+name+' . $df[1]];
                } else {
                    $out = $number . ' ' . $lang[$type . '_fields+name+' . $df[1]];
                }
                break;
            ...
            case 'date':
                if ($field['Default'] == 'single') {
                    ...
                    if (strtotime($value) > 0) {
                        ...
                        $out = $tags
                        ? ('<span class="grey_small">' . $lang['from'] . '</span> ' . $d_date . ' ')
                        : $d_date;
                    }
                    ...
                    if ($id) {
                        ...
                        if (strtotime($multi_field) > 0) {
                            ...
                            $out .= $tags
                            ? ('<span class="grey_small">' . $lang['to'] . '</span> ' . $d_date_to)
                            : ' - ' . $d_date_to;
                        }
                    }
                }
    ...
    public function homePageSpecialContent()
    {
        ...
        if ($tpl_settings['home_page_special_block']) {
            if ($blocks[$tpl_settings['home_page_special_block']]) {
                $special_block = $blocks[$tpl_settings['home_page_special_block']];
                unset($blocks[$tpl_settings['home_page_special_block']]);
            }

            if (!$special_block) {
                reset($blocks);
                $special_block = current($blocks);

                unset($blocks[$special_block['Key']]);
            }
    ...
    {
        global $page_info, $search_results_url;
    ...
    public function metaRelCanonical()
    {
        ...
        if (is_numeric(strpos($_SERVER['REQUEST_URI'], '?'))) {
            $current_url = $GLOBALS['domain_info']['scheme'] . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
            ...
            if ($page_info['Key'] == 'view_details') {
                ...
                if ($category && $category['ID']) {
                    ...
                    if ($page_info['Controller'] === 'account_type' && $account && $account['ID']) {
                        ...
                        $hreflang[$code] = $reefless->getPageUrl($page_info['Key'], null, $code);
                    }
                }
            }
        }
            ...
            if (empty($add_url)) {
                if ($custom) {
                    if ($customSubdomain) {
                        ...
                        $tpl_url = $first_url . '/index[pg].html';
                    } else {
                ...
                $first_url = $base . $page_info['Path'] . '/' . $add_url;

                // categoryFilter trailing html bug fix
                if (is_numeric(strpos($add_url, ':'))) {
                    $first_url .= '/';
                } else {
                    $first_url .= '.html';
                }
