...
if (!empty($listing_id)) {
    ...
    if (array_search($search_results_url, $_GET, true) || isset($_GET[$search_results_url])) {
        ...
        if ($listings) {
            ...
            $add_page_path = $listing_type['Add_page'] && $pages['al_' . $listing_type['Key']] ? $pages['al_' . $listing_type['Key']] : $pages['add_listing'];
    ...
    // browse/quick search mode
        ...
        if (is_numeric($category['ID'])) {
            ...
            if (!$category['Lock'] && !$listing_type['Admin_only']) {
                ...
                if ($category['ID'] > 0) {
                    $add_listing_link .= $config['mod_rewrite']
                    ? $add_page_path . '/' . $steps['plan']['path'] . '.html?'
                    : '?page=' . $add_page_path . '&step=' . $steps['plan']['path'] . '&';

                    $add_listing_link .= 'id=' . $category['ID'];
            ...
            if ($category['ID'] > 0) {
                ...
                $page_info['h1']               = $category['h1'] ?: $page_info['h1'];
                $page_info['name']             = $category['name'];
