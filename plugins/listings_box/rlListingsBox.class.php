...
class rlListingsBox
{
    ...
    public function getListings($type, $order, $limit = 0, $unique = 0, $by_category = 0)
    {
        ...
        $sql .= "LEFT JOIN `" . RL_DBPREFIX . "listing_types` AS `T4` ON `T3`.`Type` = `T4`.`Key` ";

        $rlHook->load('listingsModifyJoin');

        $sql .= "WHERE `T1`.`Status` = 'active' AND `T4`.`Photo` = '1' ";
        ...
        foreach ($listings as $key => $value) {
            ...
            $listings[$key]['url'] = $GLOBALS['reefless']->getListingUrl($value);
        }
        // save show date
