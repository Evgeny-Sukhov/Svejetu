...
{foreach from=$fields item='field'}
	{assign var='field' value=$field.Fields.0}
	...
	<tr>
		...
		{elseif $field.Type == 'number'}
			...
			{else}
				<input placeholder="{$lang.from}" value="{if $fVal.$fKey.from}{$fVal.$fKey.from}{/if}" class="numeric w60 field_from" type="text" name="f[{$field.Key}][from]" maxlength="{if $field.Values}{$field.Values}{else}18{/if}" /><img alt="" src="{$rlTplBase}img/blank.gif" class="between" /><input placeholder="{$lang.to}" value="{if $fVal.$fKey.to}{$fVal.$fKey.to}{/if}" class="numeric w60 field_to" type="text" name="f[{$field.Key}][to]" maxlength="{if $field.Values}{$field.Values}{else}18{/if}" />
			{/if}
