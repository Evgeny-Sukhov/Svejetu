...
if ($_GET['q'] == 'ext') {
    ...
    $search_parent = !$search_name || ($search_name && !$_GET['Search_all_levels']) ? true : false;

    if (!intval($parent)) {
        $parent = $parent ? $rlDb->getOne('ID', "`Key` = '" . $parent . "'", 'data_formats') : 0;
    }
    ...
    if ($_GET['action'] == 'add' || $_GET['action'] == 'edit') {
        ...
        if (isset($_POST['submit'])) {
            ...
            if (!empty($errors)) {
                ...
                if (!$_POST['geo_filter'] && $geo_format_data['Key'] == $f_key) {
