...
JAVASCRIPT;
     ...
     * @since 2.1.0
     *
     * @todo Remove this method when <compatible> will be > 4.8.0
     */
    public function hookApPhpIndexBottom()
    {
        ...
        if ($_POST['xjxfun'] == 'ajaxDeleteLang' && $_POST['xjxargs'][0]) {
            if (count($GLOBALS['languages']) == 2) {
                $GLOBALS['rlConfig']->setConfig('mf_multilingual_path', '0');
                $this->managePathFields(false);
            }
            /**
             * @todo Remove code when <compatible> will be > 4.8.0
             */
            else {
                $code = $rlDb->getOne('Code', "`ID` = {$_POST['xjxargs'][0]}", 'languages');
        ...
        elseif ($_POST['xjxfun'] == 'ajax_addLanguage' && $_POST['xjxargs'][0]) {
            foreach ($GLOBALS['rlXajax']->objArgumentManager->aArgs[0] as $param) {
                if ($param[0] == 'iso_code') {
                    if ($GLOBALS['config']['mf_multilingual_path']) {
                        $rlDb->addColumnToTable("Path_{$param[1]}", 'VARCHAR(255) NOT NULL AFTER `Key`', 'multi_formats');
                        $rlDb->query("ALTER TABLE `{db_prefix}multi_formats` ADD INDEX `Path_{$param[1]}` (`Path_{$param[1]}`)");
                    }

                    $this->createLangTable($param[1]);
                    break;
                }
            }
        }
        ...
        /**
         * Check the multilingual path fields consistency after the new language import try was performed
         *
         * @todo Remove code when <compatible> will be > 4.8.0
         */
        if ($GLOBALS['cInfo']['Controller'] == 'languages') {
            if ($_SESSION['mf_after_lang_import_trigger']) {
                foreach ($GLOBALS['languages'] as $language) {
                    if ($GLOBALS['config']['mf_multilingual_path']) {
                        $new_column = 'Path_' . $language['Code'];

                        if (!$rlDb->columnExists($new_column, 'multi_formats')) {
                            $rlDb->addColumnToTable($new_column, "VARCHAR(255) NOT NULL AFTER `Key`", 'multi_formats');
                            $rlDb->query("ALTER TABLE `{db_prefix}multi_formats` ADD INDEX `{$new_column}` (`{$new_column}`)");
                        }
                    }
                    $this->createLangTable($language['Code']);
                }

                unset($_SESSION['mf_after_lang_import_trigger']);
                return;
            }
        }
    }
    ...
    public function hookApPhpIndexBeforeController()
    {
        ...
        if ($_POST['import']) {
            $_SESSION['mf_after_lang_import_trigger'] = true;
        }
    }
     ...
     */
    public function addItem($parentKey, $names, $paths = [], $status = 'active')
    {
        ...
        $max_position = $rlDb->getOne("Position", "`Parent_ID` = {$parent_id} ORDER BY `Position` DESC", "multi_formats");
        $parent_ids = '';
     ...
     */
    public function editItem($parentKey, $key, $names, $paths = [], $status = 'active')
    {
        global $lang, $config, $rlDb, $rlValid;
    ...
    {
        global $key, $rlDb, $geo_format_data;
    ...
    {
        $pluginOptions = ['mf_geo_subdomains_type', 'mf_urls_in_sitemap', 'mf_home_in_sitemap'];
