...
switch ($_GET['action']) {
    case 'update':
        ...
        if (isset($_GET['fix']) && $config['mf_db_version'] != 'locations5') {
        ...
        if ($config['mf_db_version'] != 'locations5') {
            $update_error = true;
        }
    ...
    default:
        ...
        if (!$plugins['multiField']) {
            ...
            if ($rlDb->getOne('ID', "`Key` = 'multiField'", 'plugins')) {
                $mapping_error = $lang['locationFinder_mapping_mf_inactive_error'];
            } else {
                $mapping_error = $lang['locationFinder_mapping_mf_not_installed_error'];
            }

            $href = $rlSmarty->_tpl_vars['rlBase'] . 'index.php?controller=plugins';
            $mapping_error = preg_replace('/(\[(.*)?\])/', '<a href="' . $href . '">$2</a>', $mapping_error);
        }
        // No table case
