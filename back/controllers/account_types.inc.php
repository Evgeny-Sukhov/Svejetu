...
/* ext js action end */
    ...
    if ($_GET['action'] == 'add' || $_GET['action'] == 'edit') {
        ...
        if ($_GET['action'] == 'edit' && !$_POST['fromPost']) {
            ...
            // get account meta description
            $meta_description = $rlDb->fetch(array('Code', 'Value'), array('Key' => 'account_types+account_meta_description+' . $i_key), "AND `Status` <> 'trash'", null, 'lang_keys');
            foreach ($meta_description as $nKey => $nVal) {
                $_POST['account_meta_description'][$meta_description[$nKey]['Code']] = $meta_description[$nKey]['Value'];
            }

            // get account meta keywords
            $meta_keywords = $rlDb->fetch(array('Code', 'Value'), array('Key' => 'account_types+account_meta_keywords+' . $i_key), "AND `Status` <> 'trash'", null, 'lang_keys');
            foreach ($meta_keywords as $nKey => $nVal) {
                $_POST['account_meta_keywords'][$meta_keywords[$nKey]['Code']] = $meta_keywords[$nKey]['Value'];
            }
                ...
                if ($_GET['action'] == 'add') {
                    ...
                    if ($action = $rlActions->insertOne($data, 'account_types')) {
                        ...
                        foreach ($allLangs as $key => $value) {
                            ...
                            if (!empty($_POST['account_meta_description'][$allLangs[$key]['Code']])) {
                                $lang_keys[] = array(
                                    'Code'   => $allLangs[$key]['Code'],
                                    'Module' => 'common',
                                    'Status' => 'active',
                                    'Key'    => 'account_types+account_meta_description+' . $f_key,
                                    'Value'  => trim($_POST['account_meta_description'][$allLangs[$key]['Code']]),
                                );
                            }

                            if (!empty($_POST['account_meta_keywords'][$allLangs[$key]['Code']])) {
                                $lang_keys[] = array(
                                    'Code'   => $allLangs[$key]['Code'],
                                    'Module' => 'common',
                                    'Status' => 'active',
                                    'Key'    => 'account_types+account_meta_keywords+' . $f_key,
                                    'Value'  => trim($_POST['account_meta_keywords'][$allLangs[$key]['Code']]),
                                );
                            }
                    ...
                    foreach ($allLangs as $key => $value) {
                        ...
                        if (!empty($_POST['account_meta_description'][$allLangs[$key]['Code']])) {
                            $account_meta_description = $rlDb->fetch(array('ID'), array('Key' => 'account_types+account_meta_description+' . $f_key, 'Code' => $allLangs[$key]['Code']), null, null, 'lang_keys', 'row');
                            if (!empty($account_meta_description)) {
                                $update_phrases = array(
                                    'where'  => array(
                                        'Code' => $allLangs[$key]['Code'],
                                        'Key'  => 'account_types+account_meta_description+' . $f_key,
                            ...
                            $rlDb->query("DELETE FROM `{db_prefix}lang_keys` WHERE `Key` = 'account_types+account_meta_description+{$f_key}' AND `Code` = '{$allLangs[$key]['Code']}'");
                        }

                        /* update category listings meta keywords */
                        if (!empty($_POST['account_meta_keywords'][$allLangs[$key]['Code']])) {
                            $account_meta_keywords = $rlDb->fetch(array('ID'), array('Key' => 'account_types+account_meta_keywords+' . $f_key, 'Code' => $allLangs[$key]['Code']), null, null, 'lang_keys', 'row');
                            if (!empty($account_meta_keywords)) {
                                $update_phrases = array(
                                    'where'  => array(
                                        'Code' => $allLangs[$key]['Code'],
                                        'Key'  => 'account_types+account_meta_keywords+' . $f_key,
                            ...
                            $rlDb->query("DELETE FROM `{db_prefix}lang_keys` WHERE `Key` = 'account_types+account_meta_keywords+{$f_key}' AND `Code` = '{$allLangs[$key]['Code']}'");
                        }
                    }
