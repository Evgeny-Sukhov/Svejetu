...
if ($_GET['q'] == 'ext') {
    ...
    $sql = "SELECT SQL_CALC_FOUND_ROWS `T1`.`ID`, CONCAT(`T1`.`First_name`, ' ', `T1`.`Last_name`) AS `Name`, `T1`.`Username`, `T1`.`Type`, `T1`.`Status`, `T1`.`Mail`, `T1`.`Date`, `T1`.`Photo`, ";
        ...
        if ($_GET['action'] == 'add' || $_GET['action'] == 'edit') {
            ...
            if (isset($_POST['form_submit'])) {
                ...
                if ($_FILES['thumbnail']['name'] && is_readable($_FILES['thumbnail']['tmp_name'])) {
                    ...
                    if (!preg_match('/(\.|\/)(gif|jpe?g|png)$/i', $_FILES['thumbnail']['name'])) {
