...
{if $smarty.get.action == 'add' || $smarty.get.action == 'edit'}
    ...
    {if $smarty.get.action == 'edit'}
        <tr>
            <td class="name">{$lang.account_meta_description}</td>
            <td class="field">
                {if $allLangs|@count > 1}
                    <ul class="tabs">
                        {foreach from=$allLangs item='language' name='langF'}
                ...
                {foreach from=$allLangs item='language' name='langF'}
                    {if $allLangs|@count > 1}<div class="tab_area{if !$smarty.foreach.langF.first} hide{/if} {$language.Code}">{/if}
                        <input type="text" name="account_meta_description[{$language.Code}]" value="{$sPost.account_meta_description[$language.Code]}" class="w350" maxlength="255" />                        
                        <div>
                        <select>
        ...
        <tr>
            <td class="name">{$lang.account_meta_keywords}</td>
            <td class="field">
                {if $allLangs|@count > 1}
                    <ul class="tabs">
                        {foreach from=$allLangs item='language' name='langF'}
                ...
                {foreach from=$allLangs item='language' name='langF'}
                    {if $allLangs|@count > 1}<div class="tab_area{if !$smarty.foreach.langF.first} hide{/if} {$language.Code}">{/if}
                        <input type="text" name="account_meta_keywords[{$language.Code}]" value="{$sPost.account_meta_keywords[$language.Code]}" class="w350" maxlength="255" />                        
                        <div>
                        <select>
                            <option value="0">{$lang.select}</option>
                            {foreach from=$fields item="field"}
                ...
                <div id="account_abb">
                    {foreach from=$listing_types item='l_type'}
                        ...
                        <div class="option_padding">
                            ...
                            <label><input {if $l_type.Key|in_array:$sPost.abilities}checked="checked"{/if} type="checkbox" name="abilities[]" value="{$l_type.Key}" {if $smarty.get.action == 'edit' && !$l_type.Deny_uncheck_ability}class="disabled" disabled="disabled"{/if} /> {$lang.ability_to_add|replace:$replace:$l_type.name}</label>
                        </div>
                    {/foreach}
